...
func IsDetailPage(document *goquery.Document) bool {
	detail := document.Find("div[class=box-player]")
...
func GetCategory(document *goquery.Document) []string {
	...
	descriptionBox, _ := document.Find("div[class=box-description] ul li a").Html()
	if descriptionBox == "" {
		categories = append(categories, "Giải trí")
		return categories
	}
	document.Find("div[class=box-description] ul li a").Each(func(i int, s *goquery.Selection) {
...
func GetTitle(document *goquery.Document) string {
	var title = ""
	titleElement := document.Find(".box-description h3")
